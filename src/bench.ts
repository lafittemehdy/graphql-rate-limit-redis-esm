...
function readPositiveIntegerFromEnv(name: string, fallback: number): number {
	...
	if (!Number.isInteger(parsed) || parsed <= 0) {
		throw new Error(`Invalid ${name}: ${raw}. Expected a positive integer environment value.`);
	}
...
function createBenchmarkCases(): BenchmarkCase[] {
	...
	const { asyncKeySchema, baselineSchema, defaultKeySchema, document } = createGraphQLSchemas();
	...
	return [
		{
			...
			run: () => {
				const key = defaultKeyGenerator(mockDirectiveArgs, null, {}, userContext, mockInfo);
				if (!key) {
		...
		},
		{
			...
			run: () => {
				const key = userKeyGenerator(mockDirectiveArgs, null, {}, userContext, mockInfo);
				if (!key) {
					throw new Error("Invalid benchmark key output.");
				}
			},
...
function printResults(
	...
	const caseColumnWidth = Math.max(...results.map((result) => result.name.length), 32);
	...
	const baseline = results.find((result) => result.name === "graphql: baseline (no directive)");
	if (!baseline) {
		return;
	}
	...
	for (const result of results.filter((value) => value.name.startsWith("graphql:"))) {
		if (result.name === baseline.name) {
			continue;
		}

		const overheadPercent =
			((result.medianMicroseconds - baseline.medianMicroseconds) / baseline.medianMicroseconds) *
			100;
...
async function main(): Promise<void> {
	const iterations = readPositiveIntegerFromEnv("BENCH_ITERATIONS", DEFAULT_ITERATIONS);
	const rounds = readPositiveIntegerFromEnv("BENCH_ROUNDS", DEFAULT_ROUNDS);
	const warmupIterations = readPositiveIntegerFromEnv("BENCH_WARMUP", DEFAULT_WARMUP_ITERATIONS);
	const benchmarkCases = createBenchmarkCases();

	const results: BenchmarkResult[] = [];
	for (const benchmarkCase of benchmarkCases) {
		const result = await runBenchmarkCase(benchmarkCase, iterations, rounds, warmupIterations);
		results.push(result);
	}
